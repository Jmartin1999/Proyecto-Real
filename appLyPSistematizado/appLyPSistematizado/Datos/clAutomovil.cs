...
namespace appLyPSistematizado.Datos
{
    class clAutomovil
    {
        ...
        {
            DataTable dtAutomovil = new DataTable();
            string consulta = "select Nombres,Placa,TipoV,Observaciones from Cliente inner join Automovil on(Automovil.IdCliente=Cliente.IdCliente) where Placa= '"+Placa+"'";
            dtAutomovil = objConexion.mtdDesconectado(consulta);
            return dtAutomovil;
        }
        ...
        public DataTable mtdMostrarTodo()
        {
            ...
            string consulta = "select  Cliente.Documento,Cliente.Nombres,Cliente.Telefono,Placa,TipoV,Observaciones from Automovil inner join Cliente on (Automovil.IdAutomovil=Automovil.IdCliente) where Placa='"+Placa+"'";
